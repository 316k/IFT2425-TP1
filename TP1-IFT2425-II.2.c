...
void stack_push(stack* s, double x, double y) {
    stack_cell* new_cell = (stack_cell*) malloc(sizeof(stack_cell));
...
#define NB_ITERATIONS_MAX 200

double diverge(double x0, double y0, float** Graph2D, int length, int width) {
    
    double x = x0, y = y0, mod_z = 0, xnext, ynext;
    int i, j;
    
    stack* s = (stack*) malloc(sizeof(stack));
    s->top = NULL;

    for(i=0; i<NB_ITERATIONS_MAX; i++) {
    ...
    // Incrémente les cases de la trajectoire
    while(s->top != NULL) {
        x = s->top->x;
        y = s->top->y;

        j = (int) (x * (width - 1.0) / 2.0 + width/1.35);
        i = (int) (y * (length - 1.0) / 2.0 + length/2.0);
...
//--------------------------------------------------------------------------------
    ...
    for(i=0;i<length * 5;i++)
        for(j=0;j<width * 5;j++) {
            x = 2.0 * (j/5.0 - width/1.35) / (width - 1.0);
            y = 2.0 * (i/5.0 - length/2.0) / (length - 1.0);
            
            diverge(x, y, Graph2D, length, width);
        }
