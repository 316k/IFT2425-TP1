...
static inline double df_cheloue_mais_qui_marche(double cmv) {
    return (f(cmv + epsilon) - f(cmv)) / epsilon;
}

static inline double df(double c) {
    double h = 0, g = 0, dh = 0, dg = 0;
    ...
    return (dg*h - dh*g)/pow(h, 2) + 1.0 / (c * c);
}
...
//--------------------------------------------------------------------------------
    ...
    do {
        ...
        printf("CORNICHONS %f ::: %f %f\n", cmv, df(cmv), df_cheloue_mais_qui_marche(cmv));
